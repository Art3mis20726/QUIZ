...
const typeOfQuestion=await Questions.findOne({$and:[{type},{owner:req.user?._id}]})
if(!typeOfQuestion){
...
const correctans=asyncHandler(async(req,res)=>{
  const{type,question,selectedOption}=req.body;
  if(!type){
    throw new ApiError(400, "Please provide type of question");}
    if(!selectedOption){
    ...
    console.log(typeOfQuestion.questions)
